...
class Template
{
	...
	*/
	...
	{		
		if (strpos($blockname, '.') !== false)
		{
			...
			if ($s_row_count > 0)
			//	unset($str[$blocks[$blockcount]][($s_row_count - 1)]['S_LAST_ROW']); // commentaire chr  
				$str[$blocks[$blockcount]][($s_row_count - 1)]['S_LAST_ROW'] = false; // ajout chr	
		...
		{
			// Top-level block.
			$vararray['S_FIRST_ROW'] = false; // ajout chr
			...
			$vararray['S_LAST_ROW'] = true;
			if ($s_row_count > 0)
				//unset($this->_tpldata[$blockname][($s_row_count - 1)]['S_LAST_ROW']);  // commentaire chr
				$this->_tpldata[$blockname][($s_row_count - 1)]['S_LAST_ROW'] = false; // ajout chr			
	...
	{	
		...
		if ($mode == 'insert')
		{
			...
			if ($key >= sizeof($this->_tpldata[$blockname]))
			{
				...
				// unset($this->_tpldata[$blockname][($key - 1)]['S_LAST_ROW']); // commentaire chr
				$this->_tpldata[$blockname][($key - 1)]['S_LAST_ROW'] = false; // ajout chr
			}
			else if ($key === 0)
			{
	...
	*/
	private function compile($code, $no_echo = false, $echo_var = '')
	{
		...
		$config = array();  // ajout chr
		$config['tpl_allow_php'] = true; // ajout chr
